...
{
	if(isObject(%hat = %obj.hatModel))
	{
	
		%hat.unhidenode(ConstructionHelmetBuds);
		%hat.setnodecolor(ConstructionHelmetBuds,getRandomBotPantsColor());
		%hat.unhidenode(ConstructionHelmet);
		%hat.setnodecolor(ConstructionHelmet,"0.8 0.8 0.1 1");
	}

	if(isObject(%pack = %obj.packModel))
	{
		%pack.unhidenode(ConstructionVest);
		%pack.setnodecolor(ConstructionVest,"0.8 0.8 0.1 1");
	}
}