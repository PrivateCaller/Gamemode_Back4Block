datablock fxDTSBrickData (BrickZombieWitch_HoleSpawnData : BrickCommonZombie_HoleSpawnData)
{
    ...
    iconName = "Add-Ons/Gamemode_Left4Block/add-ins/player_l4b/icons/icon_witch";
...
function ZombieWitchHoleBot::onCollision(%this, %obj, %col, %fade, %pos, %norm)
{
    ...
    if(%col.getType() & $TypeMasks::PlayerObjectType && minigameCanDamage(%obj,%col) && checkHoleBotTeams(%obj,%col))
    {
        ...
        if(!%obj.hLoopActive)
        {
            ...
            {
                chatMessageTeam(%col.client,'fakedeathmessage',"<color:FFFF00>" @ %col.client.name SPC "<bitmapk:Add-Ons/Gamemode_Left4Block/add-ins/player_l4b/icons/ci_skull2>" SPC %obj.getDatablock().hName);
...
function ZombieWitchHoleBot::OnDamage(%this,%obj,%am)
{
    ...
    if(isObject(getMiniGameFromObject(%obj)))
    {
        ...
        {
            chatMessageTeam(%attacker.client,'fakedeathmessage',"<color:FFFF00>" @ %attacker.client.name SPC "<bitmapk:Add-Ons/Gamemode_Left4Block/add-ins/player_l4b/icons/ci_skull2>" SPC %obj.getDatablock().hName);
                    ...
                    if(%obj.StartleCount > 7)
                    {
                        ...
                        if(%obj.StartleCount > 8)
                        {
                            ...
                            if(%obj.StartleCount == 10)
                            {
                                ...
                                {
                                    chatMessageTeam(%target.client,'fakedeathmessage',"<color:FFFF00>" @ %target.client.name SPC "<bitmapk:Add-Ons/Gamemode_Left4Block/add-ins/player_l4b/icons/ci_witchclose>" SPC %obj.getDatablock().hName);
