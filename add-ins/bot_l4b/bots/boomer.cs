   ...
   if(%damageAmt)
   {
      ...
      if(%col.getType() & $TypeMasks::PlayerObjectType)
      {
			...
			if(%col.getClassName() $= "Player")
			{
				...
				{
					chatMessageTeam(%col.client,'fakedeathmessage',"<color:FFFF00>" @ %obj.sourceObject.getDatablock().hName SPC "<bitmapk:Add-Ons/Gamemode_Left4Block/add-ins/bot_l4b/icons/ci_boomer2>" SPC %col.client.name);
        ...
        if(%targetid.getType() & $TypeMasks::PlayerObjectType)
        {
			if(checkHoleBotTeams(%this.sourceObject,%targetid) && miniGameCanDamage(%this.sourceObject,%targetid))
			{
				...
				else
				{
					...
					{
						chatMessageTeam(%targetid.client,'fakedeathmessage',"<color:FFFF00>" @ %this.sourceObject.getDatablock().hName SPC "<bitmapk:Add-Ons/Gamemode_Left4Block/add-ins/bot_l4b/icons/ci_boomer2>" SPC %targetid.client.name);
