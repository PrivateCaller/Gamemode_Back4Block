...
package Interactive_BrickSet
{
	function fxDTSBrickData::onColorChange (%data, %brick)
	{
		if(isObject(%brick.healthlocker))
		%brick.healthlocker.setnodecolor("ALL",getwords(getColorIdTable(%brick.colorid),0,2) SPC "1");

		if(isObject(%brick.ammocrate))
		%brick.ammocrate.setnodecolor("ALL",getwords(getColorIdTable(%brick.colorid),0,2) SPC "1");
...
activatepackage(Interactive_BrickSet);