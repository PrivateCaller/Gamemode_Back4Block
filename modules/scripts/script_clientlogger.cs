...
function L4B_storeClientData(%client)
{
	...
	if(%client.getField(getWord($L4B_ReferenceAppearanceTags,%i)) $= $L4B_ReferenceAppearance[%i]) 
	{
	...
	for(%i = 0; %i < %statsTagCount; %i++)
	{
		...
		%file.writeLine(%statsReference SPC %client.getField(%statsReference));
	}
	...
	for(%i = 0; %i < %appearanceTagCount; %i++)
	{
		...
		%file.writeLine(%appearanceReference SPC %client.getField(%appearanceReference));
	}
