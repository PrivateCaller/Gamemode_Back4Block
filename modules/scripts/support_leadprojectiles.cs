//function to use
//pass in target object, projectile velocity, projectile gravity mod (%proj.gravityMod * %proj.isBallistic), and projectile initial position
//returns NORMAL vector projectile should be shot at - you will need to scale it up by the velocity you want the projectile to take
function getProjectileVector(%target, %projVel, %projGravityMod, %initialPosition)
{
    ...
    else //need to lead target
    {
        ...
        return vectorNormalize(vectorSub(%iterFinalPos, %initialPosition));
    }
}
...
//test function/event
function fxDTSBrick::spawnAimedProjectile(%brick, %speed, %projectile, %targetObj)
{
    ...
    %vec = vectorScale(getProjectileVector(%targetObj, %speed, %projectile.gravityMod * %projectile.isBallistic, %brick.position), %speed);
    %brick.spawnProjectile(%vec, %projectile, "", 1);
}

//registerOutputEvent("FxDTSBrick", "spawnAimedProjectile", "int 0 200 10" TAB "datablock ProjectileData" TAB "string 200 150");