...
function L4B_storeClientSnapshots()
{
    ...
    jettisonWriteFile("config/server/Left4Block/loggedplayers.json", "object", %root_object);
}
...
function L4B_loadClientSnapshots()
{
	...
	if(!isFile("config/server/Left4Block/loggedplayers.json") || jettisonReadFile("config/server/Left4Block/loggedplayers.json"))
	{
		return true;
	}
